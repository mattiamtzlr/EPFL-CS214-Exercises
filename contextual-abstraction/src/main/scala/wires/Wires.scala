...
trait Decoder[T]:
  def decode(js: ujson.Value): util.Try[T]
...
given WireFormat[ujson.Value] with
  def encode(t: Value): Value = t
  def decode(js: Value): Try[Value] = Success(js)
...
case class CastWire[T1, T2: WireFormat](enc: T1 => T2, dec: T2 => Try[T1]) extends WireFormat[T1]:
